...
import io.delta.sharing.server.config.SSLConfig
...
object BaseTestResource {
  ...
  def defaultActions(metadataConfig: Map[String, String] = defaultMetadata) = Seq(Create(metadataConfig = metadataConfig), AddData(), Bump())
  ...
  case class AddData(timestamp: LocalDateTime = now.minusMinutes(1), numRecords: Int = 4) extends Action
...
trait BaseTestResource {
  ...
  def setupTestTables(): File = {
          ...
          SchemaConfig(
            ...
            java.util.Arrays.asList(
              TableConfig("table1", createAndPopulateTable(s"$basePath/table1",
                actions = Seq(Create(yesterday), AddData(yesterday), Bump(yesterday)),
    ...
    serverConfig.setDefaultFormat(format)
