...
object LocalDeltaTestResource extends BaseTestResource {
  ...
  override def createAndPopulateTable(
    ...
    def commit[T <: Action](
      ...
      println(" \n"*3)
      println(s"$actions => Version: ${snapshot.getVersion}")
      val fileContents = fs.open(filePath)
      println(filePath)
      println(IOUtils.toString(fileContents, "UTF-8"))
      println(" \n"*3)
      commitResult
    }
      ...
      .foreach {
        case Create(ts, metadataConfig, metadataId) =>
          ...
          val metadata = Metadata
            ...
            .schema(structType)
            .partitionColumns(
              java.util.Arrays.asList(partitionColumns.map(_.getName): _*)
            )
          ...
          changes.foreach { change =>
            ...
            val writer = AvroParquetWriter
              .builder[GenericRecord](
                HadoopOutputFile.fromPath(cdcFile, hadoopConf)
              )
              .withConf(hadoopConf)
              .withSchema(schema)
              .build()
        ...
        case AddData(ts, numRecords) =>
          ...
          val writer = AvroParquetWriter
            .builder[GenericRecord](
              HadoopOutputFile.fromPath(newDataFile, hadoopConf)
            )
            .withConf(hadoopConf)
            .withSchema(schema)
            .build()
