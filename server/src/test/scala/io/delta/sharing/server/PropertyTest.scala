    ...
    ZLayer.scoped {
      for {
        ...
        serverConfig = ServerConfig(
          ...
          authorization = Authorization("dapi5e3574ec767ca1548ae5bbed1a2dc04d"),
    ...
    ZLayer.scoped {
      ZIO.acquireRelease {
        for {
          ...
          _ <- ZIO.attempt {
            // Update the Hadoop configuration to authenticate with Kerberos.
            ...
            hadoopConf.set("dfs.namenode.kerberos.principal", s"nn/${endpoints.namenodeContainerName.take(12)}@HADOOP.LOCAL")
