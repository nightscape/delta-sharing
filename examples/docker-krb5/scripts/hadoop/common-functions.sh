...
function create_tls_keystore() {
    ...
    keytool -delete -alias ${SSL_ALIAS} -keystore ${SSL_TRUSTSTORES_PATH}/truststore.jks -storepass ${STORE_PASSWD} 2>/dev/null || true
    ...
    keytool -importcert -alias ${SSL_ALIAS} \
            -file ${SSL_KEYSTORES_PATH}/${SSL_ALIAS}.cer \
            -keypass ${STORE_PASSWD} -storepass ${STORE_PASSWD} \
            -keystore ${SSL_TRUSTSTORES_PATH}/truststore.jks \
